...
public class PageInterceptor implements Interceptor {
	...
	protected final Logger logger = Logger.getLogger(PageInterceptor.class);
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (mappedStatement.getId().matches(pageSqlId)) {
			...
			if (parameterObject == null) {
				...
				if (page.isStatisTotal()) {
					Connection connection = (Connection) invocation.getArgs()[0];
					// 重设分页参数里的总页数等
					setPageInfo(sql, connection, mappedStatement, boundSql, page);
				}
			}
		}
		// 将执行权交给下一个拦截器
	 ...
	 * @param page
	 */
	private void setPageInfo(String sql, Connection connection,
			MappedStatement mappedStatement, BoundSql boundSql, PageInfo page) {
		// 记录总记录数
