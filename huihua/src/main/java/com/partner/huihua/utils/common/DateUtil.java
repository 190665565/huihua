...
public class DateUtil {
	 ...
	 * @return 日期字符串
	 */
	public static String TimestampToString(Timestamp time, DateStyle dateStyle) {
		String dateString = null;
		if (dateStyle != null) {
			dateString = DateToString(new Date(time.getTime()), dateStyle.getValue());
		}
		return dateString;
	}
	 ...
	 * @param date 指定对比的日期
	 * @param days 天数, 正数表示之前多少天, 正数表示之后多少天 
	 * @return
	 */
	 ...
	 * @param days 相关的天数, 正数表示之前多少天, 正数表示之后多少天 
	 * @return
	 */
	...
	/**
	 * 用yyyy-MM-dd HH:mm:ss格式化日期类型
	 * @author rainyhao 
	 * @since 2015-3-10 上午9:47:14
	 * @param date 日期
	 * @return
	 */
	...
	/**
	 * 取最小日期
	 * @param min_proceeds_day
	 * @param min_confirm_day
	 * @return
	 */
	public static String getMinDay(String min_proceeds_day, String min_confirm_day){
		String result = null;
		if(min_proceeds_day==null && min_confirm_day==null){
			return result;
		}else if(min_proceeds_day==null  && min_confirm_day!=null){
			result = min_confirm_day;
		}else if(min_proceeds_day!=null  && min_confirm_day==null){
			result = min_proceeds_day;
		}else{
			if(min_proceeds_day.compareTo(min_confirm_day)<0){
				result = min_proceeds_day;
			}else{
				result = min_confirm_day;
			}
	...
	/**
	 * 获取start_day和今天中间经历的天,左闭又开区间
	 * @param start_day
	 * @param pattern
	 * @return
	 */
	public static List<String> getDaysBetweenStart2Today(String start_day ,String pattern){
		SimpleDateFormat sf = new SimpleDateFormat(pattern);
		List<String> day_list = new ArrayList<String>();
		String today = formatNow(pattern);
		try{
			Calendar c_start = Calendar.getInstance();
			c_start.setTime(sf.parse(start_day));
			Calendar c_end = Calendar.getInstance();
			c_end.setTime(sf.parse(today));
			if(start_day.compareTo(today)<0){
				day_list.add(start_day);
			}
			
			while(true){
				c_start.add(Calendar.DAY_OF_MONTH, 1);
				if(sf.format(c_start.getTime()).compareTo(today)>=0){
					break;
				}
				day_list.add(sf.format(c_start.getTime()));
			}
		}catch(Exception e){
			
		}
		Collections.sort(day_list);
		return day_list;
	}
	/**
	 * 获取指定格式的 昨天
...
//		System.out.println(getYesterday("yyyyMMdd"));
		System.out.println(DateToString(new Date(),DateStyle.YYYY_MM_DD_HH_MM));
	}
}
